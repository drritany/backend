...
import { ICart, IClientInfo, ICustomerSpec, IOrder } from "./interfaces";
...
<html lang="en">
  ...
  const status = await sendEmail(SUPPORT_EMAIL, SUPPORT_PASSWORD, recipientEmail, "Dr Ritany Newsletter Subscription", undefined, template)
  return status
}
 ...
 * @returns The status of the sent email, whether successful or not
 */
 export const sendInquiryEmail = (recipientEmail: string, recipientName: string): any => {
  const template: string = `
  <!DOCTYPE html>
<html lang="en">
  <head>
  ...
  <body>
    <main>
      ...
      <div class="message">
        <span id="salute">Dear ${recipientName},</span>
        <span id="body">
  ...
  const status = sendEmail(SUPPORT_EMAIL, SUPPORT_PASSWORD, recipientEmail, "Dr Ritany Inquiry", undefined, template)
  return status
}
...
<html lang="en">
  ...
  const status = sendEmail(SUPPORT_EMAIL, SUPPORT_PASSWORD, order.customerSpec.email, "Dr Ritany Cart Order", undefined, template)
  return status
}
...
export const auth = new google.auth.GoogleAuth({
  keyFile: "drritanyCredentials.json",
  scopes: ["https://www.googleapis.com/auth/spreadsheets"],
});
...
export const spreadsheetId: string = process.env.SHEET_ID1!;
